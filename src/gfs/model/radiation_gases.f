...
!      real (kind=kind_phys), parameter :: co2vmr_def = 284.9e-6 ! 1850
!      real (kind=kind_phys), parameter :: n2ovmr_def = 0.275e-6 ! 1850
!      real (kind=kind_phys), parameter :: ch4vmr_def = 0.79e-6  ! 1850
!      real (kind=kind_phys), parameter :: o2vmr_def  = 0.209    ! 1850
!      real (kind=kind_phys), parameter :: covmr_def  = 1.50e-8  ! 1850
!      real (kind=kind_phys), parameter :: f11vmr_def = 0.0      ! 1850
!      real (kind=kind_phys), parameter :: f12vmr_def = 0.0      ! 1850
!      real (kind=kind_phys), parameter :: f22vmr_def = 0.0      ! 1850
!      real (kind=kind_phys), parameter :: cl4vmr_def = 0.0      ! 1850
!      real (kind=kind_phys), parameter :: f113vmr_def= 0.0      ! 1850
!
!  ---  co2 2-d monthly data and global mean from observed data
      real (kind=kind_phys) :: co2vmr_def = 284.7e-6 
      real (kind=kind_phys) :: n2ovmr_def =  0.275e-6  
      real (kind=kind_phys) :: ch4vmr_def =  0.790e-6  
      real (kind=kind_phys) :: o2vmr_def  =  0.209
      real (kind=kind_phys) :: covmr_def  = 1.50e-8
      real (kind=kind_phys) :: f11vmr_def = 0.0e-10
      real (kind=kind_phys) :: f12vmr_def = 0.0e-10
      real (kind=kind_phys) :: f22vmr_def = 0.0e-10
      real (kind=kind_phys) :: cl4vmr_def = 0.0e-10
      real (kind=kind_phys) :: f113vmr_def= 0.0e-11

      real (kind=kind_phys), allocatable :: co2vmr_sav(:,:,:)
      real (kind=kind_phys)              :: co2_glb, gco2cyc(12)
      real (kind=kind_phys)              :: n2o_glb,ch4_glb,f11_glb
      real (kind=kind_phys)              :: f12_glb,f22_glb,cl4_glb
      real (kind=kind_phys)              :: f113_glb
...
!          =2: use obs co2 monthly data with 2-d variation
      ...
      namelist/radiation_gases_nml/co2vmr_def, n2ovmr_def, ch4vmr_def, 
     &         o2vmr_def, covmr_def, f11vmr_def, f12vmr_def, 
     &         f22vmr_def, cl4vmr_def, f113vmr_def

!  ---  public interfaces

      public  gasinit, getgases, getozn, init_radiation_gases


! =================
      contains
! =================
      subroutine init_radiation_gases
        if (initialized) return
        open(10,file='gfs_namelist')
        read(10,nml=radiation_gases_nml)
        write(*,nml=radiation_gases_nml)
        close(10)
      end subroutine init_radiation_gases
!-----------------------------------
      subroutine gasinit                                                &
!...................................
...
!!!!!edit by Dr. Rashmi Kakatkar : ghg data is inside gfs_fix folder and
!filenames are also changed 
      data  cfile0 / 'ghg_fix/ghgforcingdecadal_glob.txt' /
      data  cfile1 / 'ghg_fix/ghgforcingdecadal_2004.txt' /
...
!  --- ... check to see if requested co2 data file existed
  ...
  24        format(i4,4x,i4,a48)
            ...
            Lab_dowhile1 : do while ( i >= iyr1 )
              read (NICO2CN, *) jyr, co2g1,n2og1,ch4g1,f11g1,f12g1,
     &                          f22g1,cl4g1,f113g1    
              if ( i == iyr .and. iyr == jyr ) then
                co2_glb =  co2g1* 1.0e-6
                n2o_glb =  n2og1* 1.0e-9
                ch4_glb =  ch4g1* 1.0e-9
                f11_glb =  f11g1* 1.0e-12
                f12_glb =  f12g1* 1.0e-12
                f22_glb =  f22g1* 1.0e-12
                cl4_glb =  cl4g1* 1.0e-12
               f113_glb = f113g1* 1.0e-12
                if ( ICO2 == 2 ) then
     ...
     &                                 co2_glb
                if ( me == 0 ) print *,'   N2O data for year',iyear,    &
     &                                 n2o_glb
                if ( me == 0 ) print *,'   CH4 data for year',iyear,    &
     &                                 ch4_glb
                if ( me == 0 ) print *,'  cfc11 data for year',iyear,    &
     &                                 f11_glb
                if ( me == 0 ) print *,'  cfc12 data for year',iyear,    &
     &                                 f12_glb
                if ( me == 0 ) print *,'  cfc22 data for year',iyear,    &
     &                                 f22_glb
                if ( me == 0 ) print *,'  ccl4 data for year',iyear,    &
     &                                 cl4_glb
                if ( me == 0 ) print *,'  fc113 data for year',iyear,    &
     &                                 f113_glb
                exit Lab_dowhile1
              else
...
!            rate = 10.00  * (iyear - iyr)   ! avg rate for testing purpose --Roxy 
          else
            rate = 0.0
          endif
...
!  ---  use obs co2 global annual mean value only
        ...
        do k = 1, LMAX
          do i = 1, IMAX
            ...
            gasdat(i,k,2) =  n2o_glb + gco2cyc(kmonsav)
            gasdat(i,k,3) =  ch4_glb + gco2cyc(kmonsav)
            gasdat(i,k,6) =  f11_glb + gco2cyc(kmonsav)
            gasdat(i,k,7) =  f12_glb + gco2cyc(kmonsav)
            gasdat(i,k,8) =  f22_glb + gco2cyc(kmonsav)
            gasdat(i,k,9) =  cl4_glb + gco2cyc(kmonsav)
           gasdat(i,k,10) = f113_glb + gco2cyc(kmonsav)
          enddo
        enddo
